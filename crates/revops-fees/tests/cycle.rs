...
fn parse_cycle_state(v: &OValue) -> ChannelCycleState {
    ChannelCycleState {
        last_revenue_rate: getf(v, "last_revenue_rate"),
        last_fee_ppm: geti(v, "last_fee_ppm"),
        trend_direction: geti(v, "trend_direction"),
        step_ppm: geti(v, "step_ppm"),
        last_update: geti(v, "last_update"),
        last_broadcast_at: geti(v, "last_broadcast_at"),
        consecutive_same_direction: geti(v, "consecutive_same_direction"),
        is_sleeping: getb(v, "is_sleeping"),
        sleep_until: geti(v, "sleep_until"),
        stable_cycles: geti(v, "stable_cycles"),
        last_broadcast_fee_ppm: geti(v, "last_broadcast_fee_ppm"),
        last_state: gets(v, "last_state"),
        forward_count_since_update: geti(v, "forward_count_since_update"),
        last_volume_sats: geti(v, "last_volume_sats"),
        congestion_active: getb(v, "congestion_active"),
        congestion_quiet_cycles: geti(v, "congestion_quiet_cycles"),
        congestion_entry_fee_ppm: geti(v, "congestion_entry_fee_ppm"),
        pending_target_ppm: geti(v, "pending_target_ppm"),
        last_gossip_refresh: geti(v, "last_gossip_refresh"),
        dynamic_htlcmin_baseline_msat: opt_i64(v, "dynamic_htlcmin_baseline_msat"),
    }
}
...
fn parse_fee_state(v: &OValue) -> ChannelFeeState {
    ...
    ChannelFeeState {
        thompson,
        pid,
        last_revenue_rate: getf(sc, "last_revenue_rate"),
        last_fee_ppm: geti(sc, "last_fee_ppm"),
        last_broadcast_fee_ppm: geti(sc, "last_broadcast_fee_ppm"),
        last_update: geti(sc, "last_update"),
        last_broadcast_at: geti(sc, "last_broadcast_at"),
        last_state: gets(sc, "last_state"),
        is_sleeping: getb(sc, "is_sleeping"),
        sleep_until: geti(sc, "sleep_until"),
        stable_cycles: geti(sc, "stable_cycles"),
        forward_count_since_update: geti(sc, "forward_count_since_update"),
        last_volume_sats: geti(sc, "last_volume_sats"),
        last_gossip_refresh: geti(sc, "last_gossip_refresh"),
        last_vegas_multiplier: getf(sc, "last_vegas_multiplier"),
...
fn three_channel_synthetic_cycle_end_to_end() {
    ...
    let mut cyc_a = ChannelCycleState {
