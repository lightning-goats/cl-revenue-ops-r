...
CREATE TABLE IF NOT EXISTS rust_boltz_attempts (
    id INTEGER PRIMARY KEY,
    request_id TEXT NOT NULL UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_rust_boltz_attempts_outcome
    ON rust_boltz_attempts(outcome);

CREATE TABLE IF NOT EXISTS rust_boltz_reservations (
    id INTEGER PRIMARY KEY,
    attempt_request_id TEXT NOT NULL,
    reserved_sats INTEGER NOT NULL,
    reserved_at INTEGER NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_rust_boltz_reservations_status
    ON rust_boltz_reservations(status);

CREATE TABLE IF NOT EXISTS rust_boltz_cooldowns (
    channel_id TEXT PRIMARY KEY,
    last_action_ts INTEGER NOT NULL
);

CREATE TABLE IF NOT EXISTS rust_boltz_ignores (
    swap_id TEXT PRIMARY KEY,
    note TEXT NOT NULL,
    added_at INTEGER NOT NULL
);

CREATE TABLE IF NOT EXISTS rust_boltz_journal (
    swap_id TEXT PRIMARY KEY,
...
// ---------------------------------------------------------------------------
// Task 63: durable Boltz attempt/reservation rails + operational state
// ---------------------------------------------------------------------------

/// One Boltz submission intent, written durably BEFORE any subprocess
/// spawn. `argv_digest` is the sha256 of the REDACTED argv (never raw
/// values); `estimated_fee_sats` is the budget currency a swap reserves.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BoltzAttempt {
    pub request_id: String,
    /// 'loop_in' | 'loop_out' | 'chainswap' | 'refund' | 'claim' |
    /// 'withdraw' (CHECK-enforced).
    pub kind: String,
    pub channel_id: Option<String>,
    pub amount_sats: i64,
...
/// One TERMINAL Boltz settlement (exactly once per attempt).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BoltzSettle {
    pub request_id: String,
    /// 'not_submitted' | 'rejected' | 'committed' | 'outcome_unknown'.
    pub outcome: String,
    pub outcome_detail: Option<String>,
...
/// One attempt whose terminal outcome was never recorded (or -- via
/// [`quarantined_boltz_attempts`] -- whose reservation is quarantined).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct UnresolvedBoltzAttempt {
    pub id: i64,
    pub request_id: String,
    pub kind: String,
    pub channel_id: Option<String>,
...
/// One durable journal row (the Python JSON file's replacement).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BoltzJournalEntry {
    pub swap_id: String,
...
/// Insert one Boltz attempt AND its `active` fee reservation atomically.
pub fn insert_boltz_attempt(conn: &Connection, attempt: &BoltzAttempt) -> Result<i64> {
    let tx = conn
        .unchecked_transaction()
        .context("begin boltz attempt txn")?;
    tx.execute(
        "INSERT INTO rust_boltz_attempts
             (request_id, kind, channel_id, amount_sats, estimated_fee_sats,
              argv_digest, submitted_at)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        params![
            attempt.request_id,
            attempt.kind,
            attempt.channel_id,
            attempt.amount_sats,
            attempt.estimated_fee_sats,
            attempt.argv_digest,
            attempt.submitted_at
        ],
    )
    .context("insert boltz attempt (UNIQUE request_id)")?;
    let attempt_id = tx.last_insert_rowid();
    tx.execute(
        "INSERT INTO rust_boltz_reservations
             (attempt_request_id, reserved_sats, reserved_at, status)
         VALUES (?1, ?2, ?3, 'active')",
        params![
            attempt.request_id,
    ...
    .context("insert active boltz reservation")?;
    tx.commit().context("commit boltz attempt txn")?;
    Ok(attempt_id)
}

/// Apply one TERMINAL Boltz settlement atomically, exactly once.
pub fn settle_boltz_attempt(conn: &Connection, settle: &BoltzSettle) -> Result<()> {
    let tx = conn
        .unchecked_transaction()
        .context("begin boltz settle txn")?;
    let updated = tx
        .execute(
            "UPDATE rust_boltz_attempts
             SET outcome = ?2, outcome_detail = ?3, swap_id = COALESCE(?4, swap_id),
                 completed_at = ?5
             WHERE request_id = ?1 AND outcome IS NULL",
            params![
                settle.request_id,
        ...
        .context("settle boltz attempt outcome")?;
    if updated == 0 {
        anyhow::bail!(
            "boltz attempt {} is already terminal (or unknown); refusing a second \
             terminal write",
            settle.request_id
        );
    }
    tx.execute(
        "UPDATE rust_boltz_reservations
         SET status = ?2, settled_sats = ?3, resolved_at = ?4
         WHERE attempt_request_id = ?1 AND status = 'active'",
        params![
            settle.request_id,
    ...
    .context("flip boltz reservation")?;
    tx.commit().context("commit boltz settle txn")?;
    Ok(())
}

fn boltz_attempt_rows(stmt: &mut rusqlite::Statement<'_>) -> Result<Vec<UnresolvedBoltzAttempt>> {
    let rows = stmt
        .query_map([], |row| {
            Ok(UnresolvedBoltzAttempt {
                id: row.get(0)?,
                request_id: row.get(1)?,
                kind: row.get(2)?,
                channel_id: row.get(3)?,
                amount_sats: row.get(4)?,
                estimated_fee_sats: row.get(5)?,
                submitted_at: row.get(6)?,
            })
        })
        .context("query boltz attempts")?
        .collect::<rusqlite::Result<Vec<_>>>()
        .context("read boltz attempts")?;
    Ok(rows)
}

/// Every Boltz attempt whose terminal outcome was never recorded.
pub fn unresolved_boltz_attempts(conn: &Connection) -> Result<Vec<UnresolvedBoltzAttempt>> {
    let mut stmt = conn
        .prepare(
            "SELECT id, request_id, kind, channel_id, amount_sats, estimated_fee_sats,
                    submitted_at
             FROM rust_boltz_attempts
             WHERE outcome IS NULL
             ORDER BY id ASC",
        )
        .context("prepare unresolved boltz attempts")?;
    boltz_attempt_rows(&mut stmt)
}

/// Every attempt whose reservation is QUARANTINED (a swap that MAY have
/// been created). These block the pending-swap gate across restarts.
pub fn quarantined_boltz_attempts(conn: &Connection) -> Result<Vec<UnresolvedBoltzAttempt>> {
    let mut stmt = conn
        .prepare(
            "SELECT a.id, a.request_id, a.kind, a.channel_id, a.amount_sats,
                    a.estimated_fee_sats, a.submitted_at
             FROM rust_boltz_attempts a
             JOIN rust_boltz_reservations r ON r.attempt_request_id = a.request_id
             WHERE r.status = 'quarantined'
             ORDER BY a.id ASC",
        )
        .context("prepare quarantined boltz attempts")?;
    boltz_attempt_rows(&mut stmt)
}

/// Sats held against the Boltz fee budget: ACTIVE + QUARANTINED
/// reservations at or after `since`.
pub fn active_boltz_reserved_sats(conn: &Connection, since: i64) -> Result<i64> {
    conn.query_row(
        "SELECT COALESCE(SUM(reserved_sats), 0)
         FROM rust_boltz_reservations
         WHERE status IN ('active', 'quarantined') AND reserved_at >= ?1",
        params![since],
        |row| row.get(0),
    )
    .context("sum active boltz reservations")
}

/// Durable per-channel cooldown upsert (survives restarts -- the Python
/// in-memory parity gap this table closes).
pub fn set_boltz_cooldown(conn: &Connection, channel_id: &str, last_action_ts: i64) -> Result<()> {
    conn.execute(
        "INSERT INTO rust_boltz_cooldowns (channel_id, last_action_ts)
         VALUES (?1, ?2)
         ON CONFLICT(channel_id) DO UPDATE SET last_action_ts = excluded.last_action_ts",
        params![channel_id, last_action_ts],
    )
    .context("upsert boltz cooldown")?;
    Ok(())
}

/// All durable cooldowns, channel-ordered.
pub fn boltz_cooldowns(conn: &Connection) -> Result<Vec<(String, i64)>> {
    let mut stmt = conn
        .prepare(
            "SELECT channel_id, last_action_ts FROM rust_boltz_cooldowns
             ORDER BY channel_id ASC",
        )
        .context("prepare boltz cooldowns")?;
    let rows = stmt
        .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))
        .context("query boltz cooldowns")?
        .collect::<rusqlite::Result<Vec<_>>>()
        .context("read boltz cooldowns")?;
    Ok(rows)
}

/// Operator ignore for an external swap id.
pub fn add_boltz_ignore(conn: &Connection, swap_id: &str, note: &str, added_at: i64) -> Result<()> {
    conn.execute(
        "INSERT INTO rust_boltz_ignores (swap_id, note, added_at)
         VALUES (?1, ?2, ?3)
         ON CONFLICT(swap_id) DO UPDATE SET note = excluded.note",
        params![swap_id, note, added_at],
    )
    .context("insert boltz ignore")?;
    Ok(())
}

pub fn remove_boltz_ignore(conn: &Connection, swap_id: &str) -> Result<()> {
    conn.execute(
        "DELETE FROM rust_boltz_ignores WHERE swap_id = ?1",
        params![swap_id],
    )
    .context("delete boltz ignore")?;
    Ok(())
}

/// All ignores as (swap_id, note, added_at).
pub fn boltz_ignores(conn: &Connection) -> Result<Vec<(String, String, i64)>> {
    let mut stmt = conn
        .prepare(
            "SELECT swap_id, note, added_at FROM rust_boltz_ignores
             ORDER BY swap_id ASC",
        )
        .context("prepare boltz ignores")?;
    let rows = stmt
        .query_map([], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)))
        .context("query boltz ignores")?
        .collect::<rusqlite::Result<Vec<_>>>()
        .context("read boltz ignores")?;
    Ok(rows)
}

/// Upsert one journal record (latest record wins; pruning is the owner's
/// explicit `revops_boltz::journal` kernel, never a retention sweep).
pub fn upsert_boltz_journal(
    conn: &Connection,
    swap_id: &str,
    ...
    let record_json = serde_json::to_string(record).context("serialize journal record")?;
    conn.execute(
        "INSERT INTO rust_boltz_journal (swap_id, record_json, source, recorded_at)
         VALUES (?1, ?2, ?3, ?4)
         ON CONFLICT(swap_id) DO UPDATE SET
             record_json = excluded.record_json,
             source = excluded.source,
             recorded_at = excluded.recorded_at",
        params![swap_id, record_json, source, recorded_at],
    )
    .context("upsert boltz journal")?;
    Ok(())
}

/// The full journal, oldest recorded first.
pub fn boltz_journal(conn: &Connection) -> Result<Vec<BoltzJournalEntry>> {
    let mut stmt = conn
        .prepare(
            "SELECT swap_id, record_json, source, recorded_at
             FROM rust_boltz_journal
             ORDER BY recorded_at ASC, swap_id ASC",
        )
        .context("prepare boltz journal")?;
    let rows = stmt
        .query_map([], |row| {
            let swap_id: String = row.get(0)?;
            let record_json: String = row.get(1)?;
            let source: String = row.get(2)?;
            let recorded_at: i64 = row.get(3)?;
            Ok((swap_id, record_json, source, recorded_at))
        })
        .context("query boltz journal")?
        .collect::<rusqlite::Result<Vec<_>>>()
        .context("read boltz journal")?;
    rows.into_iter()
        .map(|(swap_id, record_json, source, recorded_at)| {
            Ok(BoltzJournalEntry {
                swap_id,
                record: serde_json::from_str(&record_json).context("parse journal record")?,
                source,
                recorded_at,
            })
        })
...
/// Remove journal rows the prune kernel dropped (called with the exact
/// keep-set the owner computed via `revops_boltz::journal`).
pub fn delete_boltz_journal_except(conn: &Connection, keep_swap_ids: &[String]) -> Result<usize> {
    // SQLite's parameter limit makes a NOT IN unusable for large sets;
    // journal is capped at 200 entries by the kernel, so build it inline.
    let placeholders = keep_swap_ids
        .iter()
        .map(|_| "?")
        .collect::<Vec<_>>()
        .join(",");
    let sql = if keep_swap_ids.is_empty() {
        "DELETE FROM rust_boltz_journal".to_string()
    } else {
        format!("DELETE FROM rust_boltz_journal WHERE swap_id NOT IN ({placeholders})")
    };
    let deleted = conn
        .execute(&sql, rusqlite::params_from_iter(keep_swap_ids.iter()))
        .context("prune boltz journal")?;
    Ok(deleted)
}
